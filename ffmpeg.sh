...
PKG_CONFIG_PATH=$INSTALL_DDIR/lib/pkgconfig \
./configure --prefix=$INSTALL_DDIR --enable-shared --enable-nonfree \
		...
		--enable-libtheora --enable-libvorbis  --enable-libx264  --enable-libx265 --enable-libvpx --enable-libxvid \
		--extra-cflags=-I/usr/local/cpffmpeg/include/ --extra-ldflags=-L/usr/local/cpffmpeg/lib \
		 --enable-version3 --extra-version=syslint  --pkg-config-flags="--static"
   make -j$cpu
